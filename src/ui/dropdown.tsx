...
export function Dropdown(props: MenuRootProps) {
  return (
    <MenuRoot {...props}>
      <div class="relative w-fit">{props.children}</div>
    </MenuRoot>
  );
}
...
export function DropdownButton({
  ...
  children,
  ...props
}: { asChild?: true; children: JSXElement; class?: never } | ButtonProps) {
  return asChild ? (
    ...
    <MenuPositioner>
      <MenuContent
        ...
        class={clsx(
          props.class,
          // Base styles
          "isolate w-max min-w-52 rounded-xl p-1",
...
export function DropdownItem({
  value = "",
  ...props
}: (HTMLArkProps<"a"> | HTMLArkProps<"button">) & {
  value?: string;
...
export function DropdownHeader(props: HTMLArkProps<"div">) {
  return (
    <div
      {...props}
      class={clsx(props.class, "col-span-5 px-3.5 pb-1 pt-2.5 sm:px-3")}
    />
  );
}
...
export function DropdownLabel(props: HTMLArkProps<"div">) {
  return (
    <div
      {...props}
      data-slot="label"
...
export function DropdownDescription(props: HTMLArkProps<"div">) {
  return (
    <div
      ...
      class={clsx(
        props.class,
        "col-start-2 row-start-2 text-sm/5 text-zinc-500 group-data-[focus]:text-white dark:text-zinc-400 sm:text-xs/5 forced-colors:group-data-[focus]:text-[HighlightText]",
      )}
    />
  );
}
...
export function DropdownShortcut({
  ...
  ...props
}: { keys: string | string[] } & HTMLArkProps<"kbd">) {
  return (
    <span
      {...props}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split("")).map((char, index) => (
        <kbd
          class={clsx([
            "min-w-[2ch] text-center font-sans capitalize text-zinc-400 group-hover:text-white forced-colors:group-hover:text-[HighlightText]",
