		...
		<TableRowContext.Provider value={value}>
			<tr
				...
				class={clsx(
					props.class,
					props.href &&
						"has-[[data-row-link][data-focus]]:-outline-offset-2 hover:bg-hover-popover has-[[data-row-link][data-focus]]:outline has-[[data-row-link][data-focus]]:outline-2 has-[[data-row-link][data-focus]]:outline-blue-500",
					striped && "even:bg-popover/20 hover:bg-hover-card",
					!striped && "hover:bg-hover-card",
				)}
			>
				{props.children}
			</tr>
...
export function TableHeader(props: HTMLArkProps<"th">) {
	...
	return (
		<th
			...
			class={clsx(
				props.class,
				"border-b border-b-border px-4 py-2 font-medium last:pr-4 first:pl-4",
...
export function TableCell(props: HTMLArkProps<"td">) {
	...
	return (
		<td
			...
			class={clsx(
				props.class,
				"relative px-2 last:pr-4 first:pl-4",
				!striped && "border-popover border-b",
				grid && "border-l border-l-popover first:border-l-0",
				dense ? "py-2.5" : "py-4",
				!bleed && "sm:last:pr-2 sm:first:pl-2",
			)}
		>
